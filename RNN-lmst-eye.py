...
#del a,countlabel,images_train,images_valid,index,j,video1,video2,videos_path1,videos_path2,videos_path3,videos_path4,videocount,y_train,y_valid,images_test,y_test
...
history=model.fit(X_train, dummy_y_train, batch_size=32,epochs=40, validation_data=(X_valid, dummy_y_valid))
...
modellstm.add(LSTM(32, return_sequences=True, input_shape=(2,1)))  
modellstm.add(Dropout(0.5))# returns a sequence of vectors of dimension 32
...
history2=modellstm.fit(X_train, dummy_y_train, batch_size=32,epochs=30, validation_data=(X_valid, dummy_y_valid))
...
# Save the weights
model.save_weights('model_weights_CNN-LMST_eye.h5')
# Save the model architecture
with open('model_architecture_CNN-LMST_eye.json', 'w') as f:
    f.write(model.to_json())
#
modellstm.save_weights('model_weights_CNN-LMST_eye.h5')
# Save the model architecture
with open('model_architecture_CNN-LMST_eye.json', 'w') as m:
    m.write(model.to_json())
#end=datetime.datetime.now()
#elapsed=end-start
plot_model(model, to_file='CNN-LMST_eye.png')
plot_model(modellstm, to_file='CNN-LMST_eye.png')
...
probs=[]    
for p in range (0,762):
    #762 is the size of validation
    if probs1[p,0]>0.5:
        probs.append(1)
    else:
        probs.append(0)
...
probstest=[]    
for p in range (0,942):
    #942 is the size of validation
    if probs2[p,0]>0.5:
        #probabilty more than 0.5
        probstest.append(1)
    else:
        probstest.append(0)
